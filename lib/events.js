...
EventEmitter.on = async function* (emitter, name, { signal } = {}) {
  var notAborted = !signal?.aborted;
  if (!notAborted) throw new Error('Aborted');
  while (notAborted) yield await EventEmitter.once(emitter, name, { signal });
};
